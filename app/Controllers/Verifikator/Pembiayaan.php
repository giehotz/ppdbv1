...
class Pembiayaan extends BaseController
{
    ...
    public function bayar($siswaId)
    {
        ...
        if (empty($jumlah) || $jumlah <= 0 || empty($tanggal)) {
        ...
        $this->pembayaranModel->save([
            ...
            'diverifikasi_oleh' => session()->get('nama_lengkap'),
        ]);
        ...
        } elseif (!empty($tagihanIds)) {
            $ids = array_filter(array_map('intval', explode(',', $tagihanIds)));
            foreach ($ids as $id) {
                $this->tagihanModel->markAsLunas($id);
            }
        } else {
            $sisaBayar = (int) $jumlah;
            $unpaid = $this->tagihanModel->where('siswa_id', $siswaId)->where('status_bayar', 'belum')->orderBy('harga_satuan', 'ASC')->findAll();
        ...
        session()->setFlashdata('success', 'Pembayaran berhasil dicatat dan tagihan ditandai lunas.');
        return redirect()->to('/verifikator/pembiayaan/siswa/' . $siswaId);
    }
    ...
    public function updateStatusBayar($siswaId)
    {
        ...
        catat_log('Update Status Bayar', "Verifikator memperbarui status bayar siswa ID $siswaId");
    ...
    public function kuitansi($siswaId)
    {
        ...
        $totalTagihan = $this->tagihanModel->getTotalTagihan($siswaId);
        $totalBayar   = $this->pembayaranModel->getTotalBayar($siswaId);
        $tagihan      = $this->tagihanModel->getTagihanBySiswa($siswaId);
        $riwayatBayar = $this->pembayaranModel->getRiwayatBySiswa($siswaId);
        ...
        $pdf->generate('verifikator/pembiayaan/kuitansi_pdf', [
            'siswa'         => $siswa,
            'tagihan'       => $tagihan,
            'totalTagihan'  => $totalTagihan,
            'totalBayar'    => $totalBayar,
