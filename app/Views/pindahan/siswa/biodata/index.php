<?= $this->extend($layout ?? 'pindahan/layouts/siswa_pindahan') ?>

<?= $this->section('title') ?>Biodata Siswa Pindahan<?= $this->endSection() ?>
...
<div class="flex items-center gap-2 w-full">
    ...
    <div class="min-w-0">
        <h1 class="text-base font-bold text-gray-900 dark:text-white leading-none">Biodata Siswa Pindahan</h1>
        <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">Lengkapi identitas, data sekolah asal, serta berkas pindahan</p>
    </div>
    <?php
    $pageStatusVerif = strtolower(trim($pindahan['status_verifikasi'] ?? ''));
    $pageIsFinal = (($pindahan['status_pendaftaran'] ?? '') === 'Final') && ($pageStatusVerif !== 'ditolak');
    ?>
    <?php if ($pageIsFinal): ?>
...
<?php
$status_verifikasi = strtolower(trim($pindahan['status_verifikasi'] ?? ''));
$isFinal = (($pindahan['status_pendaftaran'] ?? '') === 'Final') && ($status_verifikasi !== 'ditolak');

$this->setData([
    'siswa'           => $pindahan,
    'isFinal'         => $isFinal,
    'isVerifikator'   => $isVerifikator ?? false,
    'formAction'      => $formAction ?? '/siswa/pindahan/biodata/update',
...
<div class="rounded-2xl border border-gray-200/80 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 overflow-hidden">
    ...
    <div class="border-b border-gray-100 bg-gray-50/50 px-3 sm:px-6 dark:border-gray-800/80 dark:bg-gray-900/50">
        <nav class="flex items-center gap-2 sm:gap-4 overflow-x-auto py-3 no-scrollbar scrollable-tabs" style="scroll-behavior: smooth; -webkit-overflow-scrolling: touch;">
            <button onclick="showTab('dataDiri')" id="tab-dataDiri" class="tab-button group inline-flex items-center gap-2 rounded-xl px-3.5 py-2.5 text-xs font-semibold transition-all duration-200 shrink-0 border border-transparent border-brand-500 bg-brand-500/10 text-brand-600 dark:bg-brand-500/20 dark:text-brand-400">
            ...
            <button onclick="showTab('alamat')" id="tab-alamat" class="tab-button group inline-flex items-center gap-2 rounded-xl px-3.5 py-2.5 text-xs font-semibold transition-all duration-200 shrink-0 border border-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-800">
            ...
            <button onclick="showTab('orangTua')" id="tab-orangTua" class="tab-button group inline-flex items-center gap-2 rounded-xl px-3.5 py-2.5 text-xs font-semibold transition-all duration-200 shrink-0 border border-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-800">
            ...
            <button onclick="showTab('asal')" id="tab-asal" class="tab-button group inline-flex items-center gap-2 rounded-xl px-3.5 py-2.5 text-xs font-semibold transition-all duration-200 shrink-0 border border-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-800">
                ...
                <span class="whitespace-nowrap">Sekolah Asal</span>
            </button>
            <button onclick="showTab('rapor')" id="tab-rapor" class="tab-button group inline-flex items-center gap-2 rounded-xl px-3.5 py-2.5 text-xs font-semibold transition-all duration-200 shrink-0 border border-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-800">
                ...
                <span class="whitespace-nowrap">Nilai Rapor</span>
            </button>
            <button onclick="showTab('berkas')" id="tab-berkas" class="tab-button group inline-flex items-center gap-2 rounded-xl px-3.5 py-2.5 text-xs font-semibold transition-all duration-200 shrink-0 border border-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-800">
                ...
                <span class="whitespace-nowrap">Berkas</span>
            </button>
        </nav>
    </div>
        ...
        <div class="rounded-2xl border border-gray-100 bg-gradient-to-r from-gray-50/80 via-white to-gray-50/80 p-4 sm:p-5 dark:border-gray-800/80 dark:from-gray-800/40 dark:via-gray-800/20 dark:to-gray-800/40 shadow-sm">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2.5">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <span class="text-xs sm:text-sm font-bold text-gray-900 dark:text-white">Status Kelengkapan Biodata</span>
                        <p class="text-[11px] text-gray-500 dark:text-gray-400" id="progressInfo">
                            ...
                            <?php else: ?>
                                <span class="text-emerald-600 dark:text-emerald-400 font-bold inline-flex items-center gap-1">
                                    <span class="material-symbols-outlined text-sm">verified</span> Formulir 100% lengkap!
                                </span>
                            <?php endif; ?>
                        </p>
                    </div>
                ...
                <div class="flex items-center gap-3 self-end sm:self-auto">
                    ...
                    <div class="flex items-baseline gap-1 text-right">
                        ...
                        <span class="text-base sm:text-lg font-black font-mono <?= ($completionPercentage ?? 0) < 100 ? 'text-amber-500 dark:text-amber-400' : 'text-emerald-500 dark:text-emerald-400' ?>" id="progressText"><?= $completionPercentage ?? 0 ?>%</span>
                    </div>
                </div>
            </div>
    ...
    <!-- Main Form Area -->
    <form action="<?= $formAction ?? base_url('siswa/pindahan/biodata/update') ?>" method="post" class="p-5 sm:p-8" id="formBiodata">
        <?= csrf_field() ?>

        <?= $this->include('siswa/biodata/_data_diri') ?>
        <?= $this->include('pindahan/siswa/biodata/_alamat') ?>
        <?= $this->include('siswa/biodata/_orang_tua') ?>
        <?= $this->include('pindahan/siswa/biodata/_asal_sekolah') ?>
        <?= $this->include('pindahan/siswa/biodata/_nilai_rapor') ?>
    </form>

    <!-- Upload Berkas & Navigation Action Bar -->
    <div class="px-5 pb-6 sm:px-8 sm:pb-8">
        <?= $this->include('pindahan/siswa/biodata/_upload_berkas') ?>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-3 mt-8 pt-6 border-t border-gray-100 dark:border-gray-800">
            ...
            <div class="flex flex-wrap items-center gap-2.5 w-full sm:w-auto justify-end">
                ...
                <?php if (isset($isVerifikator) && $isVerifikator): ?>
                    <button type="button" onclick="window.location.href='<?= base_url('verifikator/pindahan/cetak-akun/' . $pindahan['id_pindahan']) ?>'" id="btnFinalize"
...
<?= $this->include('pindahan/siswa/biodata/_scripts') ?>
<?= $this->endSection() ?>