...
<head>
    ...
    <title><?= $this->renderSection('title') ?> - <?= esc($page_title ?? 'Siswa Pindahan') ?> | <?= esc($app_alias ?? 'PPDB') ?></title>
...
>
    ...
    $sidebarMenus = [
        'Menu Pindahan' => [
            ['label' => 'Dashboard',            'icon' => 'swap_horiz',        'url' => 'siswa/pindahan/dashboard'],
            ['label' => 'Biodata',              'icon' => 'user-edit',         'url' => 'siswa/pindahan/biodata'],
            ['label' => 'Upload Berkas',        'icon' => 'file-upload',       'url' => 'siswa/pindahan/berkas'],
            ['label' => 'Status',               'icon' => 'clipboard-check',   'url' => 'siswa/pindahan/status'],
        ],
        'Cetak' => [
            ['label' => 'Cetak Formulir',       'icon' => 'print',             'url' => 'siswa/pindahan/cetak-formulir'],
            ['label' => 'Cetak Kartu',          'icon' => 'id-card',           'url' => 'siswa/pindahan/cetak-kartu'],
        ],
    ];
    ...
    $hasFoto = false;
    if (!empty($foto)) {
        $candidate = str_starts_with($foto, 'uploads/') ? $foto : 'uploads/berkas/' . $nisn . '/' . $foto;
        $fotoPath  = $candidate;
        $hasFoto   = file_exists(FCPATH . $candidate);
    }
              ...
              <div class="flex grow items-center justify-between px-4 py-3 sm:px-6 lg:py-3.5">
                ...
                <div class="flex items-center gap-2 sm:gap-3">
                  ...
                  >
                    ...
                    <svg x-show="darkMode" class="theme-toggle-sun h-5 w-5 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
                    </svg>
                    <!-- Moon Icon (shown in light mode) -->
                    <svg x-show="!darkMode" class="theme-toggle-moon h-5 w-5 text-gray-600 dark:text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  >
                    ...
                    >
                      <span class="h-9 w-9 overflow-hidden rounded-full ring-2 ring-brand-500/20 bg-brand-50 dark:bg-brand-500/15 flex items-center justify-center font-bold text-brand-600 dark:text-brand-400 text-xs">
                        ...
                        <?php else: ?>
                          <?= strtoupper(substr(session()->get('nama_lengkap') ?? 'P', 0, 1)) ?>
                        <?php endif; ?>
                      </span>
                      ...
                      <div class="hidden text-left md:block">
                        ...
                        <span class="block text-[10px] text-gray-400 dark:text-gray-500 font-mono leading-none mt-0.5">
                          <?= esc(session()->get('no_pendaftaran') ?? 'Siswa Pindahan') ?>
                        </span>
                      </div>
                    ...
                    >
                      ...
                      <div class="py-1">
                        <a
                          href="<?= base_url('siswa/pindahan/biodata') ?>"
