...
<head>
    ...
    <title>Cetak Akun Siswa Pindahan</title>
    ...
    <style>
        ...
        .page { background: #fff; width: 100%; max-width: 800px; padding: 3rem; border-radius: 1rem; box-shadow: 0 10px 15px -3px rgb(0 0 0 / 0.1); position: relative; }
        ...
        .info-value { flex: 1; font-weight: 700; color: #0f172a; word-break: break-word; }
...
<body>
    ...
    <a href="<?= base_url('verifikator/pindahan') ?>" class="back-btn">
        &larr; Kembali ke Daftar Siswa Pindahan
    </a>
    ...
    <div class="page">
        <?php if (function_exists('render_kop_surat')) : ?>
            <?= render_kop_surat() ?>
        <?php else : ?>
            <div class="header">
                <div class="header-text">
                    <h1><?= esc($web['nama_sekolah'] ?? 'SEKOLAH') ?></h1>
                    <p><?= esc($web['alamat'] ?? '') ?></p>
                </div>
            </div>
        <?php endif; ?>
        ...
        <div class="title">
            <h2>INFORMASI AKUN PENDAFTARAN SISWA PINDAHAN</h2>
        </div>
