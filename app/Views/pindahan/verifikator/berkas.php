...
<?= $this->section('title') ?>Kelola Berkas Siswa Pindahan<?= $this->endSection() ?>
<?= $this->section('page_title') ?>
<span class="material-symbols-outlined text-brand-500 mr-1">folder_open</span> Kelola Berkas Siswa Pindahan
<?= $this->endSection() ?>

<?= $this->section('content') ?>

<?php
$idPindahan = $siswa['id_pindahan'] ?? 0;
$nisnP      = preg_replace('/[^a-zA-Z0-9_-]/', '', (string) ($siswa['nisn'] ?? ''));
$namaBerkas = $siswa['nama_lengkap'] ?? '-';

$uploadedMap = $uploadedBerkas ?? [];
if (empty($uploadedMap) && !empty($berkasList)) {
    foreach ($berkasList as $bk) {
        $uploadedMap[$bk['jenis_berkas']] = $bk;
    }
}

$berkasWajibStatus = $berkasWajib ?? null;
...
<div class="mb-6 rounded-2xl border border-gray-200 bg-white p-5 md:p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="flex flex-col lg:flex-row justify-between lg:items-center gap-6">
        <div class="flex items-start sm:items-center gap-4 flex-1">
            <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-600 dark:bg-indigo-500/15 dark:text-indigo-400 shrink-0 border border-indigo-200/60 dark:border-indigo-500/20">
                <span class="material-symbols-outlined text-2xl">folder_shared</span>
            </div>
            <div class="space-y-1 flex-1 min-w-0">
                <h3 class="text-base sm:text-lg font-bold text-gray-900 dark:text-white truncate"><?= esc($namaBerkas) ?></h3>
                <p class="text-xs text-gray-500 dark:text-gray-400">
                    <span class="inline-flex rounded-lg bg-gray-100 px-2 py-0.5 font-mono text-[11px] font-bold text-gray-700 dark:bg-gray-800 dark:text-gray-300 mr-1.5">
                        <?= esc($siswa['no_pendaftaran'] ?? '-') ?>
                    </span>
                    NISN: <span class="font-mono font-bold"><?= esc($nisnP ?: '-') ?></span>
        ...
        <div class="shrink-0 rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50/70 dark:bg-gray-800/40 p-4 min-w-[200px]">
            <?php if ($berkasWajibStatus !== null) : ?>
                <div class="flex items-center justify-between text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
                    ...
                    <span class="font-mono text-brand-600 dark:text-brand-400 font-bold"><?= $berkasWajibStatus['sudah'] ?> / <?= $berkasWajibStatus['total'] ?></span>
                </div>
                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden mb-1.5">
                    <?php
                    $berkasPct = $berkasWajibStatus['total'] > 0 ? round(($berkasWajibStatus['sudah'] / $berkasWajibStatus['total']) * 100) : 0;
                    ?>
                    <div class="h-2 rounded-full transition-all duration-700 <?= $berkasPct == 100 ? 'bg-emerald-500' : 'bg-brand-500' ?>" style="width: <?= $berkasPct ?>%"></div>
                </div>
                <p class="text-[10px] text-right font-medium <?= $berkasPct == 100 ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-400 dark:text-gray-500' ?>">
                    <?= $berkasPct == 100 ? 'Lengkap &#10003;' : 'Sisa ' . count($berkasWajibStatus['kurang']) . ' berkas' ?>
                </p>
            <?php else : ?>
                <div class="flex items-center justify-between text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
                    <span>Berkas Terunggah</span>
                    <span class="font-mono text-brand-600 dark:text-brand-400 font-bold"><?= count($uploadedMap) ?> / <?= count($requiredDocs) ?></span>
                </div>
                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden mb-1.5">
                    <?php $uploadPct = count($requiredDocs) > 0 ? round((count($uploadedMap) / count($requiredDocs)) * 100) : 0; ?>
                    <div class="h-2 rounded-full transition-all duration-700 <?= $uploadPct == 100 ? 'bg-emerald-500' : 'bg-brand-500' ?>" style="width: <?= $uploadPct ?>%"></div>
                </div>
                <p class="text-[10px] text-right font-medium <?= $uploadPct == 100 ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-400 dark:text-gray-500' ?>">
                    <?= $uploadPct == 100 ? 'Lengkap &#10003;' : 'Sisa ' . (count($requiredDocs) - count($uploadedMap)) . ' berkas' ?>
                </p>
            <?php endif; ?>
        </div>
    </div>
...
<div class="space-y-4">
    <?php foreach ($requiredDocs as $jenis => $label): ?>
        <?php
        $isUploaded = isset($uploadedMap[$jenis]);
        $berkasItem = $isUploaded ? $uploadedMap[$jenis] : null;
        ...
        <div class="rounded-2xl border <?= $isUploaded ? 'border-emerald-100 dark:border-emerald-950/40' : 'border-gray-100 dark:border-gray-800' ?> bg-white p-5 shadow-sm transition-all duration-200 hover:shadow-md dark:bg-white/[0.02]">
            <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2 shrink-0">
                    <?php if ($isUploaded): ?>
                        <?= $statusBadge ?>
                        <?php if (!empty($berkasItem['path_file'])): ?>
                            <a href="<?= base_url($berkasItem['path_file']) ?>" target="_blank" rel="noopener" class="inline-flex items-center justify-center h-9 w-9 rounded-xl border border-gray-200 bg-white text-gray-700 hover:bg-gray-50 hover:text-brand-600 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 transition-colors shadow-sm" title="Lihat">
                                <span class="material-symbols-outlined text-base">visibility</span>
                            </a>
                        <?php elseif (!empty($berkasItem['nama_file'])): ?>
                            <a href="<?= base_url('uploads/berkas/' . rawurlencode($nisnP) . '/' . rawurlencode($berkasItem['nama_file'])) ?>" target="_blank" rel="noopener" class="inline-flex items-center justify-center h-9 w-9 rounded-xl border border-gray-200 bg-white text-gray-700 hover:bg-gray-50 hover:text-brand-600 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 transition-colors shadow-sm" title="Lihat">
                                <span class="material-symbols-outlined text-base">visibility</span>
                            </a>
                        <?php endif; ?>
                        <form action="<?= base_url('verifikator/pindahan/berkasDelete/' . $berkasItem['id_berkas_pindahan']) ?>" method="post" onsubmit="return confirm('Hapus berkas ini?');" class="inline m-0">
                            <?= csrf_field() ?>
            ...
            <div class="mt-3 pt-3 border-t border-gray-100 dark:border-gray-800">
                <form action="<?= base_url('verifikator/pindahan/berkasUpload/' . $idPindahan) ?>" method="post" enctype="multipart/form-data" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                    <?= csrf_field() ?>
...
<div class="mt-6 flex flex-wrap justify-between gap-3">
    <a href="<?= base_url('verifikator/pindahan/detail/' . $idPindahan) ?>" class="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-5 py-2.5 text-xs font-bold text-gray-700 shadow-sm hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 transition-colors">
        <span class="material-symbols-outlined text-base">arrow_back</span> Kembali ke Detail
    </a>
    <a href="<?= base_url('verifikator/pindahan/biodata/' . $idPindahan) ?>" class="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-blue-700 transition-colors">
        <span class="material-symbols-outlined text-base">edit_document</span> Edit Biodata
    </a>
</div>
