...
<?= $this->section('title') ?>Daftarkan Siswa Pindahan<?= $this->endSection() ?>
<?= $this->section('page_title') ?>
<span class="material-symbols-outlined text-brand-500 mr-1">swap_horiz</span> Pendaftaran Siswa Pindahan Offline (Verifikator)
<?= $this->endSection() ?>
    ...
    <div class="mb-4 rounded-xl border border-red-200 bg-red-50/80 p-4 text-red-800 shadow-sm dark:border-red-900/50 dark:bg-red-950/40 dark:text-red-300">
        <div class="flex items-start gap-3">
            ...
            <div class="flex-1 text-xs">
                <span class="font-bold block mb-1">Gagal Mendaftarkan Siswa Pindahan:</span>
...
<!-- Daftar Siswa Pindahan Terdaftar Card -->
<div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="border-b border-gray-100 px-5 py-4 dark:border-gray-800 md:px-6 flex items-center justify-between flex-wrap gap-3">
        <div>
            <h3 class="text-sm font-bold text-gray-900 dark:text-white">Daftar Siswa Pindahan Registrasi Offline</h3>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Siswa pindahan yang didaftarkan langsung oleh Verifikator. Klik "Isi Biodata" untuk melengkapi formulir pindah.</p>
        </div>
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors dark:hover:bg-white/[0.02]">
                            ...
                            <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                <div class="inline-flex items-center gap-1.5 flex-wrap justify-center">
                                    <a href="<?= base_url('verifikator/pindahan/biodata/' . $s['id_pindahan']) ?>"
                                    ...
                                    <form action="<?= base_url('verifikator/pindahan/delete/' . $s['id_pindahan']) ?>" method="post"
                                          onsubmit="return confirm('Yakin ingin menghapus siswa pindahan ini beserta seluruh berkasnya secara permanen?');"
                                          class="inline m-0 p-0">
                                        <?= csrf_field() ?>
                                        <button type="submit"
                                            class="inline-flex items-center gap-1 rounded-lg border border-red-200 bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-100 dark:border-red-900/50 dark:bg-red-950/40 dark:text-red-400 transition-colors"
    ...
    <?php else : ?>
        <div class="py-12 px-5 text-center text-gray-400 dark:text-gray-500">
            <div class="flex flex-col items-center justify-center">
                ...
                <p class="text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Belum Ada Siswa Pindahan Terdaftar</p>
                <p class="text-xs text-gray-400">Daftarkan akun siswa pindahan offline menggunakan tombol di atas.</p>
            </div>
        </div>
    <?php endif; ?>
</div>

<!-- Modal Registrasi Akun Siswa Pindahan (TailAdmin Style) -->
<div id="registerModal" class="fixed inset-0 bg-black/60 z-50 hidden flex items-center justify-center p-4 backdrop-blur-sm">
    <div class="bg-white dark:bg-gray-900 rounded-2xl max-w-2xl w-full p-6 md:p-8 shadow-theme-xl border border-gray-200 dark:border-gray-800 max-h-[90vh] overflow-y-auto transform transition-all">
        <div class="flex items-center justify-between pb-4 mb-6 border-b border-gray-100 dark:border-gray-800">
            <div>
                <h3 class="text-base font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    ...
                    <span>Registrasi Akun Siswa Pindahan Baru</span>
                </h3>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Isi data dasar untuk membuat akun siswa pindahan di sistem PPDB.</p>
            </div>
        ...
        <form action="<?= base_url('verifikator/pindahan/store') ?>" method="post" id="formRegisterSiswa" onsubmit="return validateFormRegister()">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label for="jenjang_sekolah_asal" class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Jenjang Sekolah Asal <span class="text-red-500">*</span></label>
                    <select name="jenjang_sekolah_asal" id="jenjang_sekolah_asal" required
                        class="w-full h-10 px-3.5 rounded-xl border border-gray-200 bg-gray-50/50 text-xs text-gray-900 dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:border-brand-500 focus:bg-white focus:outline-none cursor-pointer">
                        <option value="">-- Pilih Jenjang --</option>
                        <?php foreach (($semuaJenjang ?? []) as $kode => $label) : ?>
                            <option value="<?= esc($kode) ?>" <?= old('jenjang_sekolah_asal') === $kode ? 'selected' : '' ?>>
                                <?= esc($label) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
