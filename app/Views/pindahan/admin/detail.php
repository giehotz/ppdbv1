...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
    <a href="<?= base_url('admin/pindahan') ?>" class="inline-flex items-center text-gray-500 hover:text-gray-800 bg-white border border-gray-200 hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-800 dark:hover:text-white font-medium py-2 px-4 rounded-xl transition-all shadow-sm">
        <i class="fas fa-arrow-left mr-2"></i> Kembali ke Daftar
    </a>

    <div class="flex flex-wrap items-center gap-3">
        <a href="<?= base_url('admin/pindahan/cetak/' . $siswa['id_pindahan']) ?>" target="_blank" class="inline-flex items-center bg-white text-indigo-600 border border-indigo-200 hover:bg-indigo-50 dark:border-indigo-900/50 dark:bg-indigo-950/40 dark:text-indigo-400 dark:hover:bg-indigo-900/60 font-medium py-2 px-4 rounded-xl transition-all shadow-sm">
            <i class="fas fa-print mr-2"></i> Cetak Formulir
        </a>
        <form action="<?= base_url('admin/pindahan/reset-password/' . $siswa['id_pindahan']) ?>" method="post" data-confirm="Yakin ingin mereset password siswa ini? Password baru akan digenerate secara acak." class="inline-block">
            <?= csrf_field() ?>
    ...
    <div class="bg-gradient-to-r from-blue-600 to-indigo-700 px-6 py-8 md:px-10 md:py-8 text-white flex flex-col md:flex-row items-center md:items-start gap-6 relative overflow-hidden">
        ...
        <div class="text-center md:text-left z-10 flex-1">
            ...
            <div class="flex flex-wrap justify-center md:justify-start gap-4 text-blue-100 mt-3 text-sm md:text-base">
                ...
                <div class="flex items-center gap-1.5">
                    <i class="fas fa-map-marker-alt text-sm"></i> Jalur: <span class="font-semibold text-white">Pindahan</span>
                </div>
                <div class="flex items-center gap-1.5">
                    <i class="fas fa-route text-sm"></i> Jenjang: <span class="font-semibold text-white"><?= esc($siswa['jenjang_sekolah_asal'] ?? '-') ?></span>
                </div>
            </div>
        </div>
    </div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div class="p-6 flex-1">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="font-semibold text-gray-800 dark:text-white"><?= $siswa['tgl_pindahan'] ? date('d F Y • H:i', strtotime($siswa['tgl_pindahan'])) : '-' ?></p>
                </div>
                <div>
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase mb-1">Jalur Masuk</p>
                    <p class="font-semibold text-blue-600 bg-blue-50 px-3 py-1 rounded-lg inline-block">Siswa Pindahan</p>
                </div>
                <div class="sm:col-span-2">
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase mb-1">Jenjang Asal</p>
    ...
    <div class="bg-white rounded-2xl shadow-theme-xs border border-gray-200 dark:border-gray-800 dark:bg-white/[0.03] overflow-hidden flex flex-col">
        ...
        <div class="p-6 flex-1">
            <form action="<?= base_url('admin/pindahan/verify/' . $siswa['id_pindahan']) ?>" method="post">
                <?= csrf_field() ?>
...
<div class="bg-white rounded-2xl shadow-theme-xs border border-gray-200 dark:border-gray-800 dark:bg-white/[0.03] overflow-hidden mb-8">
    ...
    <div class="p-6 md:p-8">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-y-6 gap-x-8">
            ...
            <div>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Transportasi</p>
                <p class="font-semibold text-gray-800 dark:text-white"><?= esc($siswa['trans'] ?? '-') ?></p>
            </div>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div class="p-6">
            <div class="space-y-5">
                <div>
                    ...
                    <p class="font-bold text-gray-800 dark:text-white text-lg"><?= esc($siswa['nama_sekolah_asal'] ?? '-') ?> <span class="text-sm font-normal text-gray-500 dark:text-gray-400 ml-1">(<?= esc($siswa['npsn_sekolah_asal'] ?? '-') ?>)</span></p>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Jenjang</p>
                        <p class="font-semibold text-gray-800 dark:text-white"><?= esc($siswa['jenjang_sekolah_asal'] ?? '-') ?></p>
                    </div>
                    <div>
                        <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Kelas Asal</p>
                        <p class="font-semibold text-gray-800 dark:text-white"><?= esc($siswa['kelas_sekolah_asal'] ?? '-') ?></p>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-xs font-medium text-emerald-600 dark:text-emerald-400 uppercase tracking-wider mb-1"><i class="fas fa-door-open mr-1"></i>Diterima di Kelas</p>
                        <p class="font-bold text-gray-800 dark:text-white"><?= esc($siswa['kelas_diterima'] ?? '-') ?></p>
                    </div>
                </div>
                <div>
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Alamat Sekolah Asal</p>
                    <p class="font-semibold text-gray-800 dark:text-white"><?= esc($siswa['alamat_sekolah_asal'] ?? '-') ?></p>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Kota / Provinsi</p>
                        <p class="font-semibold text-gray-800 dark:text-white"><?= esc($siswa['kota_asal'] ?? '-') ?>, <?= esc($siswa['provinsi_asal'] ?? '-') ?></p>
                    </div>
                    <div>
                        <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Jurusan Asal</p>
                        <p class="font-semibold text-gray-800 dark:text-white"><?= esc($siswa['jurusan_sekolah_asal'] ?? '-') ?></p>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Tahun Masuk</p>
                        <p class="font-semibold text-gray-800 dark:text-white"><?= esc($siswa['tahun_masuk_sekolah_asal'] ?? '-') ?></p>
                    </div>
                    <div>
                        <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Tahun Keluar</p>
                        <p class="font-semibold text-gray-800 dark:text-white"><?= esc($siswa['tahun_keluar_sekolah_asal'] ?? '-') ?></p>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">No. Ijazah Asal</p>
                        <p class="font-semibold text-gray-800 dark:text-white"><?= esc($siswa['no_ijazah_sekolah_asal'] ?? '-') ?></p>
                    </div>
                    <div>
                        <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Tgl Ijazah Asal</p>
                        <p class="font-semibold text-gray-800 dark:text-white"><?= !empty($siswa['tgl_ijazah_sekolah_asal']) && $siswa['tgl_ijazah_sekolah_asal'] !== '0000-00-00' && $siswa['tgl_ijazah_sekolah_asal'] !== '0000-00-00 00:00:00' ? date('d-m-Y', strtotime($siswa['tgl_ijazah_sekolah_asal'])) : '-' ?></p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-theme-xs border border-gray-200 dark:border-gray-800 dark:bg-white/[0.03] overflow-hidden">
        <div class="bg-gray-50/50 px-6 py-4 border-b border-gray-100 dark:bg-gray-800/30 dark:border-gray-800 flex items-center gap-3">
            <i class="fas fa-flag text-indigo-500 text-lg"></i>
            <h3 class="font-bold text-gray-800 dark:text-white text-lg">Alasan Pindah</h3>
        </div>
        <div class="p-6">
            <div class="space-y-5">
                <div>
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Kategori</p>
                    <p class="font-bold text-gray-800 dark:text-white text-lg"><?= esc($siswa['alasan_pindah_kategori'] ?? '-') ?></p>
                </div>
                <div>
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Keterangan</p>
                    <p class="font-semibold text-gray-800 dark:text-white"><?= esc($siswa['alasan_pindah'] ?? '-') ?></p>
                </div>
            </div>
        </div>
    </div>
...
<!-- ==========================================
     SECTION 6: PROGRAM KESEJAHTERAAN
=========================================== -->
<div class="bg-white rounded-2xl shadow-theme-xs border border-gray-200 dark:border-gray-800 dark:bg-white/[0.03] overflow-hidden">
    <div class="bg-gray-50/50 px-6 py-4 border-b border-gray-100 dark:bg-gray-800/30 dark:border-gray-800 flex items-center gap-3">
