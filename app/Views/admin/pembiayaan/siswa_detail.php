...
<span class="material-symbols-outlined text-brand-500 mr-1">receipt_long</span> Pembiayaan - <?= esc($siswa['nama_lengkap']) ?>
<?= $this->endSection() ?>
...
<div class="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
    <a href="<?= base_url('admin/pembiayaan/siswa') ?>"
    ...
    <?php if ($statusLunas): ?>
        <a href="<?= base_url('admin/pembiayaan/siswa/' . $siswa['id_siswa'] . '/kuitansi') ?>" target="_blank"
...
<div class="mb-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="border-b border-gray-100 dark:border-gray-800 pb-4 mb-5 flex items-center justify-between">
        ...
        <span class="inline-flex items-center gap-1 rounded-full bg-brand-50 px-3 py-1 text-xs font-bold text-brand-600 dark:bg-brand-500/15 dark:text-brand-400 border border-brand-200/50 dark:border-brand-500/20">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="flex items-center justify-between p-4 rounded-2xl border transition-all <?= $t['status_bayar'] === 'lunas' ? 'border-emerald-500/50 bg-emerald-50/40 dark:border-emerald-500/20 dark:bg-emerald-500/10' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900' ?>">
                            ...
                            <div class="flex items-center gap-3 shrink-0">
                                ...
                                <?php else: ?>
                                    ...
                                    <form action="<?= base_url('admin/pembiayaan/siswa/' . $siswa['id_siswa'] . '/tagihan/hapus') ?>" method="post" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus tagihan ini?')">
                                        <?= csrf_field() ?>
                        ...
                        <div class="rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/30 p-4 transition hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-2">
                                <div class="flex items-center gap-3">
                                    <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-brand-500 text-xs font-bold text-white font-mono">
                                        <?= $no++ ?>
                                    </span>
                                    <div>
                                ...
                                <div class="flex items-center gap-2 self-end sm:self-center">
                                    ...
                                    <form action="<?= base_url('admin/pembiayaan/siswa/' . $siswa['id_siswa'] . '/pembayaran/hapus') ?>" method="post" class="inline" onsubmit="return confirm('PERINGATAN: Membatalkan transaksi ini akan menghapus catatan pembayaran dan mengembalikan status tagihan terkait menjadi BELUM LUNAS. Lanjutkan?')">
                                        <?= csrf_field() ?>
                            ...
                            <div class="ml-10 mt-3 pt-3 border-t border-gray-200/60 dark:border-gray-700">
                                    ...
                                    <div class="flex flex-wrap gap-2 mb-2">
                                        <?php foreach ($bukti as $idx => $file): ?>
                                            <div class="relative group">
                                                <a href="<?= base_url('uploads/bukti_pembayaran/' . $file) ?>" target="_blank" class="block w-16 h-16 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 hover:border-brand-500 shadow-theme-xs transition">
                                                    <img src="<?= base_url('uploads/bukti_pembayaran/' . $file) ?>" alt="Bukti" class="w-full h-full object-cover">
                                                </a>
                                                <form action="<?= base_url('admin/pembiayaan/siswa/' . $siswa['id_siswa'] . '/hapus-bukti') ?>" method="post" class="absolute -top-1.5 -right-1.5 hidden group-hover:block" onsubmit="return confirm('Hapus file bukti ini?')">
                                                    <?= csrf_field() ?>
                                                    <input type="hidden" name="id_pembayaran" value="<?= $bayar['id_pembayaran'] ?>">
                                ...
                                <form action="<?= base_url('admin/pembiayaan/siswa/' . $siswa['id_siswa'] . '/upload-bukti') ?>" method="post" enctype="multipart/form-data" class="inline-flex items-center">
                                    <?= csrf_field() ?>
                                    <input type="hidden" name="id_pembayaran" value="<?= $bayar['id_pembayaran'] ?>">
                                    <label class="inline-flex items-center gap-1.5 text-xs text-brand-600 dark:text-brand-400 hover:underline cursor-pointer font-semibold">
    ...
    <div class="space-y-6">
            ...
            <div class="space-y-4">
                ...
                <form action="<?= base_url('admin/pembiayaan/siswa/' . $siswa['id_siswa'] . '/tagihan/tambah') ?>" method="post" class="space-y-4">
                    ...
                    <div>
                        ...
                        <?php else: ?>
                            <select name="item_id" required class="w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white outline-none">
                    ...
                    <?php if (!empty($items)): ?>
                    <button type="submit" class="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-brand-500 hover:bg-brand-600 px-4 py-2.5 text-xs font-bold text-white shadow-theme-xs transition active:scale-[0.97]">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <?php else: ?>
                <form action="<?= base_url('admin/pembiayaan/siswa/' . $siswa['id_siswa'] . '/bayar') ?>" method="post" enctype="multipart/form-data" id="formBayar" class="space-y-4">
                    <?= csrf_field() ?>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <label class="inline-flex items-center gap-1.5 cursor-pointer text-xs font-bold text-brand-600 dark:text-brand-400">
                                <input type="checkbox" id="checkAllUnpaid" class="h-3.5 w-3.5 rounded border-gray-300 text-brand-600 focus:ring-brand-500">
                                <span>Pilih Semua</span>
                            </label>
                        </div>
                        <div class="space-y-2 max-h-56 overflow-y-auto pr-1">
                            <?php foreach ($unpaidItems as $unpaid): ?>
                                <label class="flex items-center justify-between p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50/60 dark:bg-gray-800/40 hover:border-brand-300 cursor-pointer transition select-unpaid-item">
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal" required value="<?= date('Y-m-d') ?>"
                               class="w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white outline-none">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="metode" class="w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white outline-none">
                    ...
                    <div>
                        ...
                        <textarea name="keterangan" rows="2" placeholder="Catatan transaksi..."
                                  class="w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white outline-none"></textarea>
                    </div>
...
// Otomatis centang semua saat halaman pertama dibuka agar cepat
