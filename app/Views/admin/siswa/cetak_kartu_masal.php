...
<head>
    ...
    <title>Cetak Kartu Masal</title>
    <!-- Include FontAwesome for icons in standard view -->
    ...
    <style>
        ...
        * {
        ...
        .a4-page {
            ...
            padding-top: <?= esc($printer['margin_atas'] ?? 0.5) ?>cm;
            padding-left: <?= esc($printer['margin_kiri'] ?? 0.5) ?>cm;
            padding-right: <?= esc($printer['margin_kiri'] ?? 0.5) ?>cm;
            
            display: grid;
            grid-template-columns: repeat(2, 86mm);
            grid-column-gap: <?= esc($printer['margin_kartu_kanan'] ?? 0.1) ?>cm;
            grid-row-gap: <?= esc($printer['margin_kartu_bawah'] ?? 0.1) ?>cm;
        ...
        /* Student Card Styles (Synced from cetak_kartu.php) */
        .card-container {
            width: 86mm;
            height: 54mm;
        ...
        .td-label { width: 30%; font-weight: 700; color: #4b5563; }
        .td-separator { width: 5px; padding-right: 3px !important; }
        ...
        /* Back Card Styles (Synced from cetak_kartu.php) */
        .back-container {
            padding: 12px;
            height: 100%;
            display: flex;
        ...
        .sig-date { font-size: 7.5px; margin-bottom: 2px; }
        .sig-title { font-size: 7.5px; font-weight: bold; margin-bottom: 5px; }
        .sig-image-wrapper {
            position: relative;
            height: 12mm;
            margin: 2px 0;
        ...
        .sig-name { font-size: 8px; font-weight: bold; text-decoration: underline; }
        .sig-nip { font-size: 7px; }
    </style>
</head>
<body>
    ...
    <div class="controls no-print">
        ...
        <div style="background: white; padding: 10px; border-radius: 8px; font-size: 12px; border: 1px solid #ccc; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            <strong>Data:</strong> <?= count($students) ?> Kartu <br>
            <strong>Mode:</strong> <?= strtoupper($print_mode) ?>
        </div>
    </div>

    <?php
    // We render up to 10 cards per A4 page (2 cols x 5 rows)
    $chunks = array_chunk($students, 10);

    foreach ($chunks as $pageIndex => $chunkStudents):
    ?>

        <?php if ($print_mode === 'front' || $print_mode === 'duplex'): ?>
        <!-- FRONT PAGE -->
        <div class="a4-page">
            <?php foreach ($chunkStudents as $index => $siswa): ?>
                <!-- Single Card Front -->
                <div class="card-container" style="<?php if (!empty($layout['bg_depan'])): ?>background-image: url('<?= base_url('uploads/kartu/' . $layout['bg_depan']) ?>'); background-size: cover;<?php endif; ?>">
                    ...
                    <div class="card-content">
                        ...
                        <div class="info-box">
                            <div class="card-label-main">KARTU PELAJAR</div>
                            <table class="details-table">
                                <tr>
                                    <td class="td-label">Nama</td><td class="td-separator">:</td>
                                    <td class="td-value"><?= esc(strtoupper($siswa['nama_lengkap'] ?? '')) ?></td>
                                </tr>
                                <tr>
                                    <td class="td-label">NISN</td><td class="td-separator">:</td>
                                    <td class="td-value"><?= esc($siswa['nisn'] ?? '-') ?></td>
                                </tr>
                                <tr>
                                    <td class="td-label">No. Daftar</td><td class="td-separator">:</td>
                                    <td class="td-value"><?= esc($siswa['no_pendaftaran'] ?? '-') ?></td>
                                </tr>
                                <tr>
                                    <td class="td-label">TTL</td><td class="td-separator">:</td>
                                    <td class="td-value">
                                ...
                                <tr>
                                    <td class="td-label">Alamat</td><td class="td-separator">:</td>
                                    <td class="td-value" style="font-size: 6.8px; line-height: 1.1;">
                                        <?= esc($siswa['alamat_siswa'] ?? '-') ?>
                                    </td>
                                </tr>
                            </table>
                        </div>
        ...
        <?php if ($print_mode === 'back' || $print_mode === 'duplex'): ?>
        <!-- BACK PAGE -->
        <div class="a4-page">
            <?php 
                /* DUPLEX LOGIC: Reverse columns to match front */
                $backChunk = [];
                for ($row = 0; $row < 5; $row++) {
                    $leftIdx = $row * 2;
                    $rightIdx = $row * 2 + 1;
                    if (isset($chunkStudents[$rightIdx])) { $backChunk[] = $chunkStudents[$rightIdx]; } 
                    else if (isset($chunkStudents[$leftIdx])) { $backChunk[] = null; }
                    if (isset($chunkStudents[$leftIdx])) { $backChunk[] = $chunkStudents[$leftIdx]; }
                }
            ?>
            <?php foreach ($backChunk as $siswa): ?>
                <?php if ($siswa === null): ?>
                    <div style="width: 86mm; height: 54mm;"></div>
                <?php else: ?>
                    <!-- Single Card Back -->
