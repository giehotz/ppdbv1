    ...
    <div class="px-6 py-5 border-b border-gray-100 bg-gray-50/50">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Pilih siswa untuk melihat detail dan mengelola pembayaran.</p>
            </div>
            <form action="<?= base_url('verifikator/pembiayaan/tambah-semua-siswa') ?>" method="post" onsubmit="return confirm('Tambahkan semua item pembiayaan ke SEMUA siswa?')">
                <?= csrf_field() ?>
                <button type="submit" class="inline-flex items-center bg-emerald-600 hover:bg-emerald-700 text-white font-medium py-2 px-4 rounded-lg transition duration-200 shadow-sm">
                    <i class="fas fa-check-double mr-2"></i> Tambah Semua ke Semua Siswa
                </button>
            </form>
        </div>
    </div>
                    ...
                    <?php else: ?>
                        <?php $no = 1; foreach ($siswaList as $s): ?>
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-sm text-center">
                                    <a href="<?= base_url('verifikator/pembiayaan/siswa/' . $s['id_siswa']) ?>" class="text-blue-600 hover:text-blue-800 transition" title="Kelola Pembayaran">
