            ...
            <div>
                <div class="flex flex-wrap items-center justify-center sm:justify-start gap-2.5">
                    ...
                    <span class="inline-flex items-center gap-1 rounded-full bg-brand-50 px-3 py-0.5 text-xs font-semibold text-brand-600 dark:bg-brand-500/15 dark:text-brand-400 border border-brand-200/60 dark:border-brand-500/20">
                        ...
                        <?= ucfirst(esc($user['level'] ?? 'Verifikator')) ?>
                    </span>
                </div>
...
<div id="tab-info" class="profile-tab-content">
    <div class="rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <form action="<?= base_url('verifikator/profile/update') ?>" method="post" class="p-6">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" value="<?= ucfirst(esc($user['level'] ?? 'Verifikator')) ?>" readonly disabled
...
<div id="tab-security" class="profile-tab-content hidden">
    <div class="rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <form action="<?= base_url('verifikator/profile/updatePassword') ?>" method="post" class="p-6 max-w-2xl">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                ...
                <div class="flex flex-col items-center justify-center p-6 rounded-2xl border border-gray-200 bg-gray-50/50 dark:border-gray-800 dark:bg-gray-800/30 text-center">
                    ...
                    <?php if (!empty($user['foto'])): ?>
                        <form action="<?= base_url('verifikator/profile/deleteFoto') ?>" method="post" class="mt-5" onsubmit="return confirm('Apakah Anda yakin ingin menghapus foto profil ini?');">
                            <?= csrf_field() ?>
                ...
                <div class="md:col-span-2 space-y-5">
                    <form action="<?= base_url('verifikator/profile/updateFoto') ?>" method="post" enctype="multipart/form-data">
                        <?= csrf_field() ?>
...
<script>
    function switchProfileTab(tabId) {
        ...
        // Save active tab
        localStorage.setItem('active_verifikator_profile_tab', tabId);
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const savedTab = localStorage.getItem('active_verifikator_profile_tab') || 'tab-info';
