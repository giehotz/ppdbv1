...
<span class="material-symbols-outlined text-brand-500 mr-1">lock_open</span> Antrean Permohonan Buka Kunci
<?= $this->endSection() ?>
...
<div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="border-b border-gray-100 px-5 py-4 dark:border-gray-800 md:px-6">
        <h3 class="text-sm font-bold text-gray-900 dark:text-white">Daftar Permohonan Buka Kunci (Tertunda)</h3>
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Siswa memohon perbaikan biodata dan meminta Verifikator membuka kembali formulir pendaftaran yang sudah difinalisasi.</p>
    </div>
            ...
            <tbody class="divide-y divide-gray-100 text-xs dark:divide-gray-800">
                <?php if (!empty($requests)) : ?>
                    <?php foreach ($requests as $r) : 
                        $inisial = mb_substr(trim($r['nama_lengkap']), 0, 1);
                    ?>
                        <tr class="hover:bg-gray-50/50 transition-colors dark:hover:bg-white/[0.02]">
                            <td class="py-3.5 px-5 whitespace-nowrap">
                                <div class="font-bold text-gray-900 dark:text-gray-200"><?= date('d M Y', strtotime($r['created_at'])) ?></div>
                            ...
                            <td class="py-3.5 px-5">
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <div>
                                        ...
                                        <span class="inline-flex rounded-md bg-gray-100 dark:bg-gray-800 px-2 py-0.5 font-mono text-[11px] font-bold text-gray-700 dark:text-gray-300 mt-0.5">
                            ...
                            <td class="py-3.5 px-5">
                                <div class="rounded-xl border border-amber-200 bg-amber-50/60 p-3 text-xs italic text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/40 dark:text-amber-200 leading-relaxed">
                                    "<?= esc($r['alasan']) ?>"
                                </div>
                            </td>
                            <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                <div class="inline-flex items-center justify-center gap-2">
                                    <form action="<?= base_url('verifikator/unlockrequest/approve/' . $r['id_request']) ?>" method="post" class="inline" data-confirm="Apakah Anda yakin menyetujui dan MEMBUKA KEMBALI form siswa ini?">
                                        <?= csrf_field() ?>
                                        <button type="submit" class="inline-flex items-center gap-1 rounded-xl bg-emerald-600 px-3.5 py-1.5 text-xs font-bold text-white shadow-theme-xs transition-colors hover:bg-emerald-700 active:scale-[0.98]">
                                    ...
                                    <form action="<?= base_url('verifikator/unlockrequest/reject/' . $r['id_request']) ?>" method="post" class="inline" data-confirm="Tolak permohonan ini? (Data siswa akan tetap TERKUNCI FINAL)">
                                        <?= csrf_field() ?>
                                        <button type="submit" class="inline-flex items-center gap-1 rounded-xl bg-red-500 px-3.5 py-1.5 text-xs font-bold text-white shadow-theme-xs transition-colors hover:bg-red-600 active:scale-[0.98]">
                ...
                <?php else : ?>
                    <tr>
                        <td colspan="4" class="py-12 px-5 text-center text-gray-400 dark:text-gray-500">
                            <span class="material-symbols-outlined text-4xl text-emerald-500/60 mb-2 block">task_alt</span>
                            <p class="text-xs font-bold text-gray-700 dark:text-gray-300">Tidak ada antrean permohonan buka kunci.</p>
                            <p class="text-xs text-gray-400 mt-0.5">Semua data pendaftaran siswa dalam kondisi aman dan terverifikasi.</p>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
