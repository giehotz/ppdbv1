...
<body class="bg-gray-100 font-sans leading-normal tracking-normal flex h-screen overflow-hidden">
    ...
    function renderMenu($menus, $linkClass, $activeClass, $currentUri) {
        $output = '';
        foreach ($menus as $groupLabel => $items) {
            if ($groupLabel !== 'Dashboard') {
                $output .= '<li class="sidebar-header px-6 py-2 text-xs font-semibold text-green-300 uppercase tracking-wider mt-4">'
                           . esc($groupLabel) . '</li>';
            }
            foreach ($items as $item) {
                ...
                $output .= '<i class="fas fa-' . $item['icon'] . ' w-6"></i>';
    ...
    <aside id="desktop-sidebar" class="w-64 bg-green-800 text-white flex-shrink-0 hidden md:flex flex-col shadow-xl z-20">
        ...
        <nav class="flex-1 overflow-y-auto py-4">
            <ul>
                <?= renderMenu($sidebarMenus, $linkClass, $activeClass, $currentUri) ?>
            </ul>
        </nav>
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <footer class="bg-white border-t border-gray-200 p-4 text-center text-sm text-gray-500">
            &copy; <?= date('Y') ?> <?= $app_alias ?? 'PPDB' ?> Online - <?= $sekolahName ?>. All rights reserved.
        </footer>
    </div>
